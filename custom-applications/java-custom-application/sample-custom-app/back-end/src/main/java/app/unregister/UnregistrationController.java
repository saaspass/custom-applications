...
@RestController
class UnregistrationController extends ApiController {

    @Autowired
    private SpService spService;

    @RequestMapping(value = "/unregister", method = POST)
    private ResponseEntity<?> unregister(@RequestHeader String authorization,
                                         @RequestBody UnregistrationRequest request) {
        try {
